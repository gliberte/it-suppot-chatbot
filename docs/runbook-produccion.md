...
`npm ci` en vez de `npm install`: instala exactamente lo que dice `package-lock.json` y nunca lo modifica, a diferencia de `npm install`. La causa real de que el lockfile quedara "sucio" en el servidor no era la plataforma: `package-lock.json` guarda una copia del campo `version` de `package.json` (`name`/`version` en la raiz del `packages`), y si se sube el `version` de `package.json` en un commit sin correr `npm install` localmente para sincronizar esa copia en el lockfile antes de comitear, cualquier `npm ci`/`npm install` posterior en el servidor detecta el desfase y lo corrige, generando un diff que bloquea el siguiente `git pull`. **Regla al preparar cualquier release: despues de cambiar `version` en `package.json`, correr `npm install` local y comitear el `package-lock.json` resultante en el mismo commit.** Si aun asi `npm ci` falla en el servidor por otra razon, corre `npm install` en local, confirma que el lockfile quedo correcto, comitea y vuelve a desplegar -- no lo resuelvas regenerando el lockfile directamente en el servidor.
...
**Trampa adicional, encontrada ya en producción:** un `ProgressBar` con `"value": 0` (o `NaN`/`undefined`) lo renderiza Teams como **indeterminado/animado** ("cargando..."), no como una barra vacía fija -- pasó con un técnico que tenía 0 tickets abiertos, mientras el resto de las barras (con valores > 0) se veían normales. La función `safeProgressBarValue()` en `server.js` manda un piso mínimo positivo (`0.01`) cuando el valor real sería cero, para forzar que siempre sea determinado.
