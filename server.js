...
/* ==========================================================================
   Resumen Ejecutivo Automático — Saludo sin enumeración de técnicos
   (Diseñado especialmente para Yariela Saucedo / Gerente IT)
   ========================================================================== */

function isExecutiveGreeting(message = '') {
  const normalized = normalizeComparableText(message);

  // Rechazar si ya viene con intención específica de herramienta
  if (/\b(cancelar|anular|ticket|mci|reporte|tecnico|bloqueo|vpn|correo|impresora|red|sap|licencia|solicitud|prestamo|mantenimiento)\b/.test(normalized)) return false;
  if (/#?\d{4,8}\b/.test(normalized)) return false;

  // Saludos ejecutivos puros
  return /^\s*(hola|hola\s+(sophia|good\s*morning)|buenos\s+d[ií]as|buenas\s+(tardes|noches)|hey|hi|buen\s+d[ií]a|que\s+hay|como\s+est[aá](s|n)?|qu[eé]\s+tal|buen[oa]s)\s*[!.]*\s*$/i.test(normalized);
}

async function handleExecutiveGreetingTurn({ message, user, onText, onCard, onWorking, responseChannel }) {
  if (!isExecutiveGreeting(message)) return false;

  const executiveProfile = getExecutiveItProfile(user);
  const isExec = executiveProfile || isSupportAdmin(user);
  if (!isExec) return false;

  const firstName = (user?.name || user?.displayName || 'Gerente').split(' ')[0];
  const hora = new Date().getHours();
  const saludo = hora < 12 ? 'Buenos días' : hora < 18 ? 'Buenas tardes' : 'Buenas noches';

  await Promise.resolve(onWorking?.(`${saludo}, ${firstName}. Preparando tu resumen ejecutivo IT...`));

  try {
    const report = await buildExecutiveItReport(user);
    const card = createExecutiveItReportCard(report, user);

    // Sección de MCI por líder (sin enumeración de técnicos individuales)
    const mciSummary = (report.mciProgress || []).slice(0, 5).map((m) => {
      const title = m.subject || m.title || 'MCI sin título';
      const progress = getMciProgressValue(m) || '—';
      const status = getDisplayName(m.status) || 'En progreso';
      return `• **${title}** — Avance: ${progress} | Estado: ${status}`;
    }).join('\n') || '(Sin MCI activas en este momento)';
    ...
    const csat = report.csatSummary || { avgScore: '5.0', displayStars: '⭐⭐⭐⭐⭐', ratingCount: 0 };
    const cats = (report.categoryDistribution || []).slice(0, 3).map((c) => `${c.category} (${c.count})`).join(', ') || '—';
    ...
    if (responseChannel === 'teams' && card) {
      ...
      const lines = [
        `**${saludo}, ${firstName} 👋**`,
        '',
        `📊 **Resumen Ejecutivo IT** — ${new Date().toLocaleDateString('es-PA', { weekday: 'long', day: 'numeric', month: 'long' })}`,
        '',
        `| Indicador | Valor |`,
        `|---|---|`,
        `| 🎫 Tickets Totales | **${totalTickets}** |`,
        `| 🔓 Tickets Abiertos | **${openCount}** |`,
        `| ⭐ CSAT Promedio | **${csat.avgScore}/5** (${csat.ratingCount} encuestas) |`,
        `| 📌 Top Categorías | ${cats} |`,
        '',
        `**🚧 Avances de MCI:**`,
        mciSummary
    ...
    console.warn('[ExecutiveGreeting] Error generando resumen:', err.message);
    const firstName2 = (user?.name || '').split(' ')[0];
    await onText?.(`${saludo}, ${firstName2}. Estoy lista. ¿En qué te puedo ayudar hoy?`);
    return true;
  }
}

  if (await handleExecutiveGreetingTurn({
    message,
    user,
    onText,
    onCard,
