...
describe('hasAssignedTechnicianScope', () => {
  it('reconoce frases naturales de un técnico sin preposición "a" explícita', () => {
    expect(hasAssignedTechnicianScope('Muéstrame mis tickets asignados')).toBe(true);
    expect(hasAssignedTechnicianScope('tickets que tengo asignados')).toBe(true);
    expect(hasAssignedTechnicianScope('lo que tengo asignado')).toBe(true);
  });

  it('sigue reconociendo las frases con preposición explícita (comportamiento previo)', () => {
    expect(hasAssignedTechnicianScope('Tickets asignados a mí')).toBe(true);
    expect(hasAssignedTechnicianScope('Mis tickets como técnico asignado')).toBe(true);
  });

  it('no activa el alcance para mensajes sin ninguna forma de "asignado"', () => {
    expect(hasAssignedTechnicianScope('¿Cuáles son mis tickets?')).toBe(false);
    expect(hasAssignedTechnicianScope('Qué tengo pendiente de resolver')).toBe(false);
  });
});
