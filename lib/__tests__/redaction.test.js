...
describe('minimizeValue', () => {
  ...
  it('conserva sophia_warnings cuando la respuesta trae request (ej. sdp_create_request/sdp_assign_request)', () => {
    const result = minimizeValue({
      request: { id: '1', subject: 'x' },
      sophia_warnings: ['"Fulano" no es un técnico nativo válido en ServiceDesk Plus.']
    });
    expect(result.sophia_warnings).toEqual(['"Fulano" no es un técnico nativo válido en ServiceDesk Plus.']);
  });

  it('conserva sophia_warnings cuando la respuesta trae requests[]', () => {
    const result = minimizeValue({
      requests: [{ id: '1' }],
      sophia_warnings: ['advertencia de ejemplo']
    });
    expect(result.sophia_warnings).toEqual(['advertencia de ejemplo']);
  });
