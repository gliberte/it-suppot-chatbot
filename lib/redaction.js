...
export function minimizeValue(value) {
  ...
  if (value.request) {
    return {
      request: minimizeRequest(value.request),
      sophia_warnings: Array.isArray(value.sophia_warnings) ? value.sophia_warnings : undefined
    };
  }
...
export function createAdaptiveCardPreview(card) {
  const texts = [];
  const visit = (value) => {
...
export function createAdaptiveCardAuditSignals(card) {
  const texts = [];
  const visit = (value) => {
    if (!value) return;
    if (Array.isArray(value)) {
